...
CLI.sendSignalToProcessName = function(signal, process_name) {
  Satan.executeRemote('sendSignalToProcessName', {
    ...
    printOut(cst.PREFIX_MSG + 'successfully sent signal %s to process name %s', signal, process_name);
    UX.processing.stop();
    speedList();
  });
};
...
CLI.sendSignalToProcessId = function(signal, process_id) {
  Satan.executeRemote('sendSignalToProcessId', {
    ...
    printOut(cst.PREFIX_MSG + 'successfully sent signal %s to process id %s', signal, process_id);
    UX.processing.stop();
    speedList();
  });
};
